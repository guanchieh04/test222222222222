...
# ✅ 檢查是否為空，防止 index -1 錯誤
if df.empty:
    st.error("❗選擇的時間區間沒有任何資料，請重新選擇。")
    st.stop()
...
##### 尋找最後 NAN值的位置
nan_indexes_MA = KBar_df['MA_long'][::-1].index[KBar_df['MA_long'][::-1].apply(pd.isna)]
last_nan_index_MA = nan_indexes_MA[0] if len(nan_indexes_MA) > 0 else -1
...
#### 尋找最後 NAN值的位置
nan_indexes_RSI = KBar_df['RSI_long'][::-1].index[KBar_df['RSI_long'][::-1].apply(pd.isna)]
last_nan_index_RSI = nan_indexes_RSI[0] if len(nan_indexes_RSI) > 0 else -1
...
##### 尋找最後 NAN值的位置
nan_indexes_BB = KBar_df['SMA'][::-1].index[KBar_df['SMA'][::-1].apply(pd.isna)]
last_nan_index_BB = nan_indexes_BB[0] if len(nan_indexes_BB) > 0 else -1
...
if choice_strategy == '<進場>: 移動平均線黃金交叉作多,死亡交叉作空. <出場>: 結算平倉(期貨), 移動停損.':
    ...
    with st.expander("<策略參數設定>: 交易停損量、長移動平均線(MA)的K棒週期數目、短移動平均線(MA)的K棒週期數目、購買數量"):
        ...
        ### 尋找最後 NAN值的位置
        nan_indexes_MA_trading = KBar_df['MA_long'][::-1].index[KBar_df['MA_long'][::-1].apply(pd.isna)]
last_nan_index_MA_trading = nan_indexes_MA_trading[0] if len(nan_indexes_MA_trading) > 0 else -1
